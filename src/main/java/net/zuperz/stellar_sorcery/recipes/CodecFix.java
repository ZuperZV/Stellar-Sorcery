...
public final class CodecFix {
    ...
    public static final Codec<FluidStack> OPTIONAL_FLUID_STACK_CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    ResourceLocation.CODEC.optionalFieldOf("fluid").forGetter(
                            fluidStack -> fluidStack.isEmpty() ? Optional.empty() : Optional.of(BuiltInRegistries.FLUID.getKey(fluidStack.getFluid()))
                    ),
                    Codec.INT.optionalFieldOf("amount", 1000
                    ).forGetter(FluidStack::getAmount)
            ).apply(instance, (fluidLocationOpt, amount) -> {
                if (fluidLocationOpt.isEmpty()) {
                    return FluidStack.EMPTY;
                }
                Fluid fluid = BuiltInRegistries.FLUID.get(fluidLocationOpt.get());
                return new FluidStack(fluid, amount);
            })
    );
}